...
void SDLText::UpdateInnerImpl()
{
    ...
    if (this->font->FontType == BaseFont::FONT_FILE)
    {
        ...
        this->SetSize((float)renderedSurface->h, (float)renderedSurface->w);
    ...
    else if (this->font->FontType == BaseFont::SPRITE_FONT)
    {
        ...
        finalTextWidth = (int)floor(this->textContent.length() * fallbackFontWidth);
        ...
        for (int i = 0; i < this->textContent.length();i++)
        {
            ...
            destRec.x = (int)currentSpaceRectangle->X;
            destRec.y = (int)currentSpaceRectangle->Y;
        ...
        this->SetSize((float)finalTextHeight, (float)finalTextWidth);
