...
void SDLGraphicEngine::Initialize(GraphicEngineInitParams* params)
{
    ...
    mainWindow = SDL_CreateWindow(params->WindowTitle.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, (int)floor(params->WindowSize->Width * RenderingScaleX), (int)floor(params->WindowSize->Height * RenderingScaleY), 0);
...
{
    SDL_Rect rec;
    rec.h = (int)object->GetHeight();
    rec.w = (int)object->GetWidth();
    rec.x = (int)object->GetOffsetPos().X;
    rec.y = (int)object->GetOffsetPos().Y;
...
void SDLGraphicEngine::DrawPrimitive(PrimitiveDrawInfo* primitive)
{
	...
	switch (primitive->CallType)
	{
		case DrawType_Line:
		{
			...
			SDL_RenderDrawLine(gameRenderer, (int)prim->xA, (int)prim->yB, (int)prim->xB, (int)prim->yB);
...
void SDLGraphicEngine::SetupDPI()
{
    ...
    this->RenderingScaleX = (float)floor(dpiX / 96);
    this->RenderingScaleY = (float)floor(dpiY / 96);
